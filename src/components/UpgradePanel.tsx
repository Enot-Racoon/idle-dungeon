...
export const UpgradePanel: React.FC = () => {
  ...
  const statUpgrades = upgrades.filter(
    (u) => !["weapon", "armor", "boots"].includes(u.type),
  );
  const gearUpgrades = upgrades.filter((u) =>
    ["weapon", "armor", "boots"].includes(u.type),
  );
  ...
  const varColor = (type: string) => {
    switch (type) {
      ...
      case "armor":
        return "#93c5fd";
      case "regen":
        return "var(--color-heal)";
      default:
        return "var(--color-shard)";
    }
  };
