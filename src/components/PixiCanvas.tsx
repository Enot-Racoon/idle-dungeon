...
export const PixiCanvas: React.FC<PixiCanvasProps> = ({ gameState }) => {
  ...
  useEffect(() => {
    const newEvents = gameState.combatEvents.filter(e => !processedEventsRef.current.has(e.id));
  ...
  useEffect(() => {
    ...
    if (prevMonsterId.current && prevMonsterId.current !== gameState.monster.id) {
      ...
      if (sceneRef.current) {
        ...
        sceneRef.current.monsterContainer.rotationQuaternion.setEulerAngles(0, 0, 0);
      }
    }
  ...
  useEffect(() => {
    ...
    app.ticker.add((delta) => {
      ...
      if (animStateRef.current.heroHitTime > 0) {
        ...
        hb.rotationQuaternion.setEulerAngles(0, 0, Math.sin(progress * Math.PI) * 15); // tilt left
      }
      ...
      if (animStateRef.current.monsterAttackTime > 0) {
        ...
        mb.rotationQuaternion.setEulerAngles(0, 0, -Math.sin(progress * Math.PI) * 20); // forward tilt
      } else {
